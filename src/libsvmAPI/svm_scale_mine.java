...
public class svm_scale_mine
{
	public svm_scale_mine(String a){
		normalizationOut=a;
	}
	private static int indexTrans;
	private static double valueTrans;
	private String normalizationOut;
	...
	public void run(String []argv) throws IOException
	{
		...
		/* pass 3: scale */
		FileWriter fw=new FileWriter(normalizationOut,false);  //
		...
		while(readline(fp) != null)
		{
			...
			{
				index = Integer.parseInt(st.nextToken());//字符串转化为整数
				value = Double.parseDouble(st.nextToken());
				for (i = next_index; i<index; i++)
					output(i, 0);
				output(index, value);
				bw.write(indexTrans + ":" + valueTrans + " ");
				next_index = index + 1;
			}
...
//		svm_scale_mine s = new svm_scale_mine();
//		s.run(argv);
	}
}
